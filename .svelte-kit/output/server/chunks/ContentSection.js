...
const Tag = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"tag " + escape(color, true) + " svelte-1xh8qlg"}">${
    ...
    return tag$1
      ? `<${tag}${spread(
          ...
          is_void(tag$1)
            ...
            : `${
                $$slots.image
                  ? `<div class="image svelte-1h3ghvu">${
                      slots.image ? slots.image({}) : ``
                    }</div>`
                  : ``
              }
  <div class="body svelte-1h3ghvu"><div class="content svelte-1h3ghvu">${
...
const BlogPostCard = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    ...
    if ($$props.tags === void 0 && $$bindings.tags && tags !== void 0)
      $$bindings.tags(tags);
    if (
      $$props.readingTime === void 0 &&
      $$bindings.readingTime &&
      readingTime !== void 0
    )
      $$bindings.readingTime(readingTime);
    if (
      $$props.showImage === void 0 &&
      $$bindings.showImage &&
      showImage !== void 0
    )
    ...
    return `${validate_component(Card, "Card").$$render(
      $$result,
      {
        ...
        target: "_self",
        additionalClass:
          "blog-post-card " + (!showImage || !coverImage ? "no-image" : ""),
      },
      {},
      {
        footer: () => {
          return `<div class="footer svelte-1v62nxu" slot="footer">${
            tags?.length
              ? `<div class="tags svelte-1v62nxu">${each(
                  tags.slice(0, 2),
                  (tag) => {
        ...
        content: () => {
          return `<div class="content svelte-1v62nxu" slot="content"><p class="title svelte-1v62nxu">${escape(
            title
          )}</p>
    ${
      readingTime
        ? `<div class="note svelte-1v62nxu">${escape(readingTime)}</div>`
        : ``
    }
    ${
      excerpt ? `<p class="text svelte-1v62nxu">${escape(excerpt)}</p>` : ``
    }</div>`;
        },
        image: () => {
          return `<div class="image" slot="image">${
            coverImage
...
const ContentSection = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
    let $$slots = compute_slots(slots);
    let { id = void 0 } = $$props;
    ...
    return `<section${add_attribute("id", id, 0)} class="${
      "content-section " + escape(align, true) + " svelte-1snaw8w"
    }"><div class="title-area svelte-1snaw8w">${
      title || description
        ? `<div class="text svelte-1snaw8w">${
            title
    ...
    ${
      $$slots["button"]
        ? `<div class="button">${slots.button ? slots.button({}) : ``}</div>`
        : ``
    }</div>
  <div class="content-area svelte-1snaw8w">${
    slots.default ? slots.default({}) : ``
  }</div>
</section>`;
  }
);
export { BlogPostCard as B, ContentSection as C, Tag as T, Card as a };
