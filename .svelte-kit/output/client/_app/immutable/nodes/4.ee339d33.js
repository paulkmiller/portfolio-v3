import {
  S as ee,
  i as te,
  s as oe,
  J as x,
  K as E,
  m as c,
  h as i,
  n,
  b as f,
  G as l,
  M,
  C as tt,
  k as v,
  a as b,
  l as k,
  c as _,
  g as C,
  v as Xe,
  d as A,
  f as et,
  D as ot,
  E as at,
  F as nt,
  y as U,
  z as F,
  A as Z,
  B as J,
  q as p,
  r as w,
} from "../chunks/index.c7992fe2.js";
import { I as Ge } from "../chunks/Image.f2f50b72.js";
function it(m) {
  let e, a, o, r;
  return {
    c() {
      (e = x("svg")),
        (a = x("path")),
        (o = x("path")),
        (r = x("path")),
        this.h();
    },
    l(s) {
      e = E(s, "svg", {
        width: !0,
        height: !0,
        "stroke-width": !0,
        viewBox: !0,
        fill: !0,
        xmlns: !0,
      });
      var d = c(e);
      (a = E(d, "path", {
        d: !0,
        stroke: !0,
        "stroke-linecap": !0,
        "stroke-linejoin": !0,
      })),
        c(a).forEach(i),
        (o = E(d, "path", {
          d: !0,
          stroke: !0,
          "stroke-linecap": !0,
          "stroke-linejoin": !0,
        })),
        c(o).forEach(i),
        (r = E(d, "path", {
          d: !0,
          stroke: !0,
          "stroke-linecap": !0,
          "stroke-linejoin": !0,
        })),
        c(r).forEach(i),
        d.forEach(i),
        this.h();
    },
    h() {
      n(a, "d", "M12 7L12 13"),
        n(a, "stroke", "currentColor"),
        n(a, "stroke-linecap", "round"),
        n(a, "stroke-linejoin", "round"),
        n(o, "d", "M12 17.01L12.01 16.9989"),
        n(o, "stroke", "currentColor"),
        n(o, "stroke-linecap", "round"),
        n(o, "stroke-linejoin", "round"),
        n(
          r,
          "d",
          "M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
        ),
        n(r, "stroke", "currentColor"),
        n(r, "stroke-linecap", "round"),
        n(r, "stroke-linejoin", "round"),
        n(e, "width", "100%"),
        n(e, "height", "100%"),
        n(e, "stroke-width", "1.5"),
        n(e, "viewBox", "0 0 24 24"),
        n(e, "fill", "none"),
        n(e, "xmlns", "http://www.w3.org/2000/svg");
    },
    m(s, d) {
      f(s, e, d), l(e, a), l(e, o), l(e, r);
    },
    p: M,
    i: M,
    o: M,
    d(s) {
      s && i(e);
    },
  };
}
class rt extends ee {
  constructor(e) {
    super(), te(this, e, null, it, oe, {});
  }
}
function st(m) {
  let e, a, o;
  return {
    c() {
      (e = x("svg")), (a = x("path")), (o = x("path")), this.h();
    },
    l(r) {
      e = E(r, "svg", {
        width: !0,
        height: !0,
        "stroke-width": !0,
        viewBox: !0,
        fill: !0,
        xmlns: !0,
      });
      var s = c(e);
      (a = E(s, "path", {
        d: !0,
        stroke: !0,
        "stroke-linecap": !0,
        "stroke-linejoin": !0,
      })),
        c(a).forEach(i),
        (o = E(s, "path", {
          d: !0,
          stroke: !0,
          "stroke-linecap": !0,
          "stroke-linejoin": !0,
        })),
        c(o).forEach(i),
        s.forEach(i),
        this.h();
    },
    h() {
      n(a, "d", "M7 12.5L10 15.5L17 8.5"),
        n(a, "stroke", "currentColor"),
        n(a, "stroke-linecap", "round"),
        n(a, "stroke-linejoin", "round"),
        n(
          o,
          "d",
          "M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
        ),
        n(o, "stroke", "currentColor"),
        n(o, "stroke-linecap", "round"),
        n(o, "stroke-linejoin", "round"),
        n(e, "width", "100%"),
        n(e, "height", "100%"),
        n(e, "stroke-width", "1.5"),
        n(e, "viewBox", "0 0 24 24"),
        n(e, "fill", "none"),
        n(e, "xmlns", "http://www.w3.org/2000/svg");
    },
    m(r, s) {
      f(r, e, s), l(e, a), l(e, o);
    },
    p: M,
    i: M,
    o: M,
    d(r) {
      r && i(e);
    },
  };
}
class lt extends ee {
  constructor(e) {
    super(), te(this, e, null, st, oe, {});
  }
}
function ht(m) {
  let e, a, o, r;
  return {
    c() {
      (e = x("svg")),
        (a = x("path")),
        (o = x("path")),
        (r = x("path")),
        this.h();
    },
    l(s) {
      e = E(s, "svg", {
        width: !0,
        height: !0,
        "stroke-width": !0,
        viewBox: !0,
        fill: !0,
        xmlns: !0,
      });
      var d = c(e);
      (a = E(d, "path", {
        d: !0,
        stroke: !0,
        "stroke-linecap": !0,
        "stroke-linejoin": !0,
      })),
        c(a).forEach(i),
        (o = E(d, "path", {
          d: !0,
          stroke: !0,
          "stroke-linecap": !0,
          "stroke-linejoin": !0,
        })),
        c(o).forEach(i),
        (r = E(d, "path", {
          d: !0,
          stroke: !0,
          "stroke-linecap": !0,
          "stroke-linejoin": !0,
        })),
        c(r).forEach(i),
        d.forEach(i),
        this.h();
    },
    h() {
      n(a, "d", "M12 11.5V16.5"),
        n(a, "stroke", "currentColor"),
        n(a, "stroke-linecap", "round"),
        n(a, "stroke-linejoin", "round"),
        n(o, "d", "M12 7.51L12.01 7.49889"),
        n(o, "stroke", "currentColor"),
        n(o, "stroke-linecap", "round"),
        n(o, "stroke-linejoin", "round"),
        n(
          r,
          "d",
          "M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
        ),
        n(r, "stroke", "currentColor"),
        n(r, "stroke-linecap", "round"),
        n(r, "stroke-linejoin", "round"),
        n(e, "width", "100%"),
        n(e, "height", "100%"),
        n(e, "stroke-width", "1.5"),
        n(e, "viewBox", "0 0 24 24"),
        n(e, "fill", "none"),
        n(e, "xmlns", "http://www.w3.org/2000/svg");
    },
    m(s, d) {
      f(s, e, d), l(e, a), l(e, o), l(e, r);
    },
    p: M,
    i: M,
    o: M,
    d(s) {
      s && i(e);
    },
  };
}
class dt extends ee {
  constructor(e) {
    super(), te(this, e, null, ht, oe, {});
  }
}
function Qe(m) {
  let e, a, o, r;
  const s = [ft, ct, ut],
    d = [];
  function y(h, g) {
    return h[0] == "info"
      ? 0
      : h[0] == "warning" || h[0] == "error"
      ? 1
      : h[0] == "success"
      ? 2
      : -1;
  }
  return (
    ~(a = y(m)) && (o = d[a] = s[a](m)),
    {
      c() {
        (e = v("div")), o && o.c(), this.h();
      },
      l(h) {
        e = k(h, "DIV", { class: !0 });
        var g = c(e);
        o && o.l(g), g.forEach(i), this.h();
      },
      h() {
        n(e, "class", "icon-wrapper svelte-sxx0n0");
      },
      m(h, g) {
        f(h, e, g), ~a && d[a].m(e, null), (r = !0);
      },
      p(h, g) {
        let I = a;
        (a = y(h)),
          a !== I &&
            (o &&
              (Xe(),
              A(d[I], 1, 1, () => {
                d[I] = null;
              }),
              et()),
            ~a
              ? ((o = d[a]),
                o || ((o = d[a] = s[a](h)), o.c()),
                C(o, 1),
                o.m(e, null))
              : (o = null));
      },
      i(h) {
        r || (C(o), (r = !0));
      },
      o(h) {
        A(o), (r = !1);
      },
      d(h) {
        h && i(e), ~a && d[a].d();
      },
    }
  );
}
function ut(m) {
  let e, a;
  return (
    (e = new lt({})),
    {
      c() {
        U(e.$$.fragment);
      },
      l(o) {
        F(e.$$.fragment, o);
      },
      m(o, r) {
        Z(e, o, r), (a = !0);
      },
      i(o) {
        a || (C(e.$$.fragment, o), (a = !0));
      },
      o(o) {
        A(e.$$.fragment, o), (a = !1);
      },
      d(o) {
        J(e, o);
      },
    }
  );
}
function ct(m) {
  let e, a;
  return (
    (e = new rt({})),
    {
      c() {
        U(e.$$.fragment);
      },
      l(o) {
        F(e.$$.fragment, o);
      },
      m(o, r) {
        Z(e, o, r), (a = !0);
      },
      i(o) {
        a || (C(e.$$.fragment, o), (a = !0));
      },
      o(o) {
        A(e.$$.fragment, o), (a = !1);
      },
      d(o) {
        J(e, o);
      },
    }
  );
}
function ft(m) {
  let e, a;
  return (
    (e = new dt({})),
    {
      c() {
        U(e.$$.fragment);
      },
      l(o) {
        F(e.$$.fragment, o);
      },
      m(o, r) {
        Z(e, o, r), (a = !0);
      },
      i(o) {
        a || (C(e.$$.fragment, o), (a = !0));
      },
      o(o) {
        A(e.$$.fragment, o), (a = !1);
      },
      d(o) {
        J(e, o);
      },
    }
  );
}
function mt(m) {
  let e,
    a,
    o,
    r,
    s = m[0] && Qe(m);
  const d = m[2].default,
    y = tt(d, m, m[1], null);
  return {
    c() {
      (e = v("div")), s && s.c(), (a = b()), y && y.c(), this.h();
    },
    l(h) {
      e = k(h, "DIV", { class: !0 });
      var g = c(e);
      s && s.l(g), (a = _(g)), y && y.l(g), g.forEach(i), this.h();
    },
    h() {
      n(e, "class", (o = "callout-block " + (m[0] ?? "") + " svelte-sxx0n0"));
    },
    m(h, g) {
      f(h, e, g), s && s.m(e, null), l(e, a), y && y.m(e, null), (r = !0);
    },
    p(h, [g]) {
      h[0]
        ? s
          ? (s.p(h, g), g & 1 && C(s, 1))
          : ((s = Qe(h)), s.c(), C(s, 1), s.m(e, a))
        : s &&
          (Xe(),
          A(s, 1, 1, () => {
            s = null;
          }),
          et()),
        y &&
          y.p &&
          (!r || g & 2) &&
          ot(y, d, h, h[1], r ? nt(d, h[1], g, null) : at(h[1]), null),
        (!r ||
          (g & 1 &&
            o !== (o = "callout-block " + (h[0] ?? "") + " svelte-sxx0n0"))) &&
          n(e, "class", o);
    },
    i(h) {
      r || (C(s), C(y, h), (r = !0));
    },
    o(h) {
      A(s), A(y, h), (r = !1);
    },
    d(h) {
      h && i(e), s && s.d(), y && y.d(h);
    },
  };
}
function gt(m, e, a) {
  let { $$slots: o = {}, $$scope: r } = e,
    { type: s = void 0 } = e;
  return (
    (m.$$set = (d) => {
      "type" in d && a(0, (s = d.type)),
        "$$scope" in d && a(1, (r = d.$$scope));
    }),
    [s, r, o]
  );
}
class pt extends ee {
  constructor(e) {
    super(), te(this, e, gt, mt, oe, { type: 0 });
  }
}
function wt(m) {
  let e;
  return {
    c() {
      e = p("Sometimes, foreshadowing is relatively obvious...");
    },
    l(a) {
      e = w(a, "Sometimes, foreshadowing is relatively obvious...");
    },
    m(a, o) {
      f(a, e, o);
    },
    d(a) {
      a && i(e);
    },
  };
}
function yt(m) {
  let e,
    a,
    o,
    r,
    s,
    d,
    y,
    h,
    g,
    I,
    ae,
    V,
    D,
    Y,
    pe,
    we,
    ne,
    L,
    ye,
    ie,
    N,
    ve,
    re,
    $,
    ke,
    S,
    be,
    _e,
    B,
    Ie,
    $e,
    G,
    Ce,
    xe,
    se,
    H,
    Ee,
    le,
    W,
    j,
    Q,
    Ae,
    De,
    he,
    T,
    je,
    P,
    Me,
    Ve,
    de,
    K,
    ue,
    O,
    We,
    ce,
    z,
    fe,
    R,
    Te,
    me;
  return (
    (I = new pt({
      props: { type: "info", $$slots: { default: [wt] }, $$scope: { ctx: m } },
    })),
    (K = new Ge({
      props: {
        src: "/images/posts/mt-sherman.webp",
        alt: "A photo of Kristen, her still insanely fit father, and the barely still-alive me atop Mt. Sherman",
      },
    })),
    (z = new Ge({
      props: {
        src: "/images/posts/bigcat_and_littlecat.jpeg",
        alt: "Socks and Dobby, aka 'Big Cat' and 'Little Cat' hanging out on the couch",
        height: "420px",
      },
    })),
    {
      c() {
        (e = v("p")),
          (a = p(
            "In 2022, my then girlfriend (now fiancé), and I decided to make the leap from living in Alexandria, Virginia to Denver, Colorado. For years, I had been colloquially dubbing Virginia, specifically NOVA, “the black hole of the East Coast”. I imagine many of my contemporaries living in expensive areas across the United States have similar names for their current homes as "
          )),
          (o = v("a")),
          (r = p("housing prices continue to rise")),
          (s = p(
            ", year over year, with seemingly no cap in sight and no action from Congress to curtail continued explosive growth in living costs. Full of hope and bravado from cursory glances at Redfin and Apartments.com, I convinced my fiancé, a life-long Coloradoan, to return to her home state so I could escape the muck and insane 90%+ humdity of Virginia and Washington D.C."
          )),
          (d = b()),
          (y = v("p")),
          (h = p(
            "“Have you ever moved across the country like this?” I recall her asking. She was right to ask, of course, because I had only ever made these sorts of drastic location changes with very little to move. And even then, I’d wager most people would agree that moving a few hours away from home is hardly what qualifies as “drastic”. So, I poured myself into all the details, finding a moving company or hiring a Uhaul, how much to set aside for cleaning costs of our current apartment, what special considerations did I need for the cats we adopted, finding a place to live in Denver, and so on. With time, I checked each item off the list, and I was suddenly $10,000 poorer for it between deposits, moving costs, cleaning costs, hotel reservations, vet visits, cat stuff, and getting our aging cars much-needed maintenance. The last thing you want is to be caught with a dead vehicle in the black of night in some forgotten part of a state you’re passing through."
          )),
          (g = b()),
          U(I.$$.fragment),
          (ae = b()),
          (V = v("h2")),
          (D = v("a")),
          (Y = v("span")),
          (pe = p("#")),
          (we = p("The Move")),
          (ne = b()),
          (L = v("p")),
          (ye = p(
            "If no one in your life loves you enough to warn you about how much of a nightmare moving can be towards the tail-end of packing up your stuff, let me be the one to say: The last 5% of your packing tasks takes just as long as the first 95%. It’s insane how much stuff you can accumulate over time! But after a same-day apartment cleaning service had finished with our apartment, we said goodbye to neighbors, packed up our cats with the bare essentials we thought we’d need for the first day or two at our new place before the moving company arrived, and set our sights westward. My fiancé and I have two cats, so we each took one, and hit the highway. We were admittedly not hustling as much as I had hoped, so we didn’t get going until ~ 2pm, so after a measily 6 hours of driving, we found ourselves going through West Virginia proper in the black of night. It was just around now that we both started to realize we hadn’t gassed up once and were both starting to get uncomfortably close to passing E."
          )),
          (ie = b()),
          (N = v("p")),
          (ve = p("And then my rear driver-side tire exploded.")),
          (re = b()),
          ($ = v("p")),
          (ke = p(
            "I’m still not sure what kind of piece of metal or aluminum beast ran into my tire to leave it in a shattered mess, but I quickly radio’d to Kristen (walkie-talkies are more reliable than cellphones when driving through areas with spotty cell coverage) that I was dead in the water and needed to change out for my spare. I will always be grateful to my father for really locking in the need to have road flares with your spare in situations like this because the woods of West Virginia are vast and dense, which only helped add to the ambiance of a moonless night. My heart raced with a mixture of fear and determination. The dense woods seemed to close in around me, and the eerie sounds of unseen creatures filled the air. Kristen and I exchanged worried glances, both of us silently questioning whether we had made the right decision to embark on this cross-country journey. I swear, the entire time I was working to change my tire out for the spare, all I could hear were banjo’s amongst the dense soundscape of chittering coyotoes or the "
          )),
          (S = v("a")),
          (be = p("Mothman")),
          (_e = p(
            " himself. Having been white-water rafting on the Chattooga River (where "
          )),
          (B = v("a")),
          (Ie = p("Deliverance")),
          ($e = p(
            " was filmed), I couldn’t shake the image of a sign on the river that said "
          )),
          (G = v("em")),
          (Ce = p("“If you hear banjos, paddle faster”")),
          (xe = p(
            " out of my minds eye. After slapping on the tire, kicking tight the lug nuts with my tire iron, tossing my destroyed tire where the spare used to live, we made for the nearest gas station where things started to calm down."
          )),
          (se = b()),
          (H = v("p")),
          (Ee = p(
            "We decided to post up for the night once we made it to Lexington, Kentucky. The rest of the trip was honestly, thankfully, uneventful. During this trip, I was awe-struck at the incomprehensible vastness of the United States. I often rescinded into my mind as I imagined the fears and troubles colonial settlers must have experienced going Westward, and how wantonly they discarded and destroyed Native American wisdom and culture along the way in their self-appointed “manifest destiny”. But eventually, we made it— we were finally in Denver."
          )),
          (le = b()),
          (W = v("h2")),
          (j = v("a")),
          (Q = v("span")),
          (Ae = p("#")),
          (De = p("Denver and Co")),
          (he = b()),
          (T = v("p")),
          (je = p(
            "I am still awe-struck at the natural beauty of Colorado and its incredible biodiversity. Denver itself is surrounded on all sides by gorgeous mountains, which of course prompts many outdoors-loving individuals to experience nature through “14ers”. A “14er” is a mountain that ascenders to 14,000’ or higher in elevation and let me tell ya— there is no shortage of mountains to choose from in Colorado. One of the most memorable experiences during our time in Denver was conquering my first 14er, "
          )),
          (P = v("a")),
          (Me = p("Mount Sherman")),
          (Ve = p(
            ". The bottom of the trail is home to several abandoned mining stations, all rusted and decayed from decades of disuse, until the easily walkable dirt trails give way to a sea of stacked flat stones that make each step a calculated effort. I had forgotten what it was like to truly push myself to my physical limit since I stopped playing Rugby, but near the summit, my body began to give up as fatigue and colorie deficiency set in. A peanut-butter bar and a gatorade later, I joined Kristen for the last leg of the trek until we had finally made it to the top. As I stood at the summit, gasping for air in the thin mountain atmosphere, I felt a sense of accomplishment unlike anything I had ever experienced before. The view from the top was breathtaking, with endless peaks stretching out before me in every direction. I could feel the sun on my face and the wind in my hair, and for the half-hour we spent resting at the summit, all the challenges of the move seemed a distant afterthought."
          )),
          (de = b()),
          U(K.$$.fragment),
          (ue = b()),
          (O = v("p")),
          (We = p(
            "Despite this, and despite our accomplishments, we found ourselves missing Virginia and the DMV area. We especially felt bad for our cats, who went from living as indoor/outdoor cats in a part of VA where they had no natural predators and could roam safely outside, with my trust that they’d come home when called, something that I trained them to do over the first 6 months of their lives with us. They were now confined to an apartment, less than 1000 square feet in size, with just me to keep them company as a remote worker. I also began to quickly miss the cultural diversity afforded to us by being so close to Washington D.C. and having immediate access to the truly world-class Metro in the DMV. But it was done and we had sunk too much money to have regrets, so we settled in, and spent 2 wonderful years in Colorado before it was off to the next adventure as Kristen transitioned into being a travelling Nurse. Now we’re in Arizona and soon after, we’ll be in Oregon. Who knows what’s next, but one thing is certain: I’m so glad we moved. If we hadn’t, we’d still likely be stuck in that black hole of a state, and I wouldn’t have conquered my first 14er, or learned how to fall on my ass 23 different ways while learning to ski."
          )),
          (ce = b()),
          U(z.$$.fragment),
          (fe = b()),
          (R = v("p")),
          (Te = p(
            "Looking back on our decision to move to Denver, I realize now that it was about more than just escaping the b”lack hole of the East Coast”. It was about pushing ourselves out of our comfort zones, embracing new experiences, and discovering what we were truly capable of. The move tested our relationship, our resilience, and our sense of adventure, but in the end, it brought us closer together and taught us valuable lessons about ourselves and each other. To anyone considering a similar life change, I would say this: Don’t let fear hold you back. Embrace the unknown, take risks, and trust in your ability to adapt and grow. You never know what incredible experiences and opportunities await you on the other side."
          )),
          this.h();
      },
      l(t) {
        e = k(t, "P", {});
        var u = c(e);
        (a = w(
          u,
          "In 2022, my then girlfriend (now fiancé), and I decided to make the leap from living in Alexandria, Virginia to Denver, Colorado. For years, I had been colloquially dubbing Virginia, specifically NOVA, “the black hole of the East Coast”. I imagine many of my contemporaries living in expensive areas across the United States have similar names for their current homes as "
        )),
          (o = k(u, "A", { href: !0, rel: !0 }));
        var X = c(o);
        (r = w(X, "housing prices continue to rise")),
          X.forEach(i),
          (s = w(
            u,
            ", year over year, with seemingly no cap in sight and no action from Congress to curtail continued explosive growth in living costs. Full of hope and bravado from cursory glances at Redfin and Apartments.com, I convinced my fiancé, a life-long Coloradoan, to return to her home state so I could escape the muck and insane 90%+ humdity of Virginia and Washington D.C."
          )),
          u.forEach(i),
          (d = _(t)),
          (y = k(t, "P", {}));
        var Be = c(y);
        (h = w(
          Be,
          "“Have you ever moved across the country like this?” I recall her asking. She was right to ask, of course, because I had only ever made these sorts of drastic location changes with very little to move. And even then, I’d wager most people would agree that moving a few hours away from home is hardly what qualifies as “drastic”. So, I poured myself into all the details, finding a moving company or hiring a Uhaul, how much to set aside for cleaning costs of our current apartment, what special considerations did I need for the cats we adopted, finding a place to live in Denver, and so on. With time, I checked each item off the list, and I was suddenly $10,000 poorer for it between deposits, moving costs, cleaning costs, hotel reservations, vet visits, cat stuff, and getting our aging cars much-needed maintenance. The last thing you want is to be caught with a dead vehicle in the black of night in some forgotten part of a state you’re passing through."
        )),
          Be.forEach(i),
          (g = _(t)),
          F(I.$$.fragment, t),
          (ae = _(t)),
          (V = k(t, "H2", { id: !0 }));
        var qe = c(V);
        D = k(qe, "A", { class: !0, title: !0, "aria-hidden": !0, href: !0 });
        var Pe = c(D);
        Y = k(Pe, "SPAN", {});
        var Ke = c(Y);
        (pe = w(Ke, "#")),
          Ke.forEach(i),
          Pe.forEach(i),
          (we = w(qe, "The Move")),
          qe.forEach(i),
          (ne = _(t)),
          (L = k(t, "P", {}));
        var ze = c(L);
        (ye = w(
          ze,
          "If no one in your life loves you enough to warn you about how much of a nightmare moving can be towards the tail-end of packing up your stuff, let me be the one to say: The last 5% of your packing tasks takes just as long as the first 95%. It’s insane how much stuff you can accumulate over time! But after a same-day apartment cleaning service had finished with our apartment, we said goodbye to neighbors, packed up our cats with the bare essentials we thought we’d need for the first day or two at our new place before the moving company arrived, and set our sights westward. My fiancé and I have two cats, so we each took one, and hit the highway. We were admittedly not hustling as much as I had hoped, so we didn’t get going until ~ 2pm, so after a measily 6 hours of driving, we found ourselves going through West Virginia proper in the black of night. It was just around now that we both started to realize we hadn’t gassed up once and were both starting to get uncomfortably close to passing E."
        )),
          ze.forEach(i),
          (ie = _(t)),
          (N = k(t, "P", {}));
        var Le = c(N);
        (ve = w(Le, "And then my rear driver-side tire exploded.")),
          Le.forEach(i),
          (re = _(t)),
          ($ = k(t, "P", {}));
        var q = c($);
        (ke = w(
          q,
          "I’m still not sure what kind of piece of metal or aluminum beast ran into my tire to leave it in a shattered mess, but I quickly radio’d to Kristen (walkie-talkies are more reliable than cellphones when driving through areas with spotty cell coverage) that I was dead in the water and needed to change out for my spare. I will always be grateful to my father for really locking in the need to have road flares with your spare in situations like this because the woods of West Virginia are vast and dense, which only helped add to the ambiance of a moonless night. My heart raced with a mixture of fear and determination. The dense woods seemed to close in around me, and the eerie sounds of unseen creatures filled the air. Kristen and I exchanged worried glances, both of us silently questioning whether we had made the right decision to embark on this cross-country journey. I swear, the entire time I was working to change my tire out for the spare, all I could hear were banjo’s amongst the dense soundscape of chittering coyotoes or the "
        )),
          (S = k(q, "A", { href: !0, rel: !0 }));
        var Ne = c(S);
        (be = w(Ne, "Mothman")),
          Ne.forEach(i),
          (_e = w(
            q,
            " himself. Having been white-water rafting on the Chattooga River (where "
          )),
          (B = k(q, "A", { href: !0, rel: !0 }));
        var He = c(B);
        (Ie = w(He, "Deliverance")),
          He.forEach(i),
          ($e = w(
            q,
            " was filmed), I couldn’t shake the image of a sign on the river that said "
          )),
          (G = k(q, "EM", {}));
        var Oe = c(G);
        (Ce = w(Oe, "“If you hear banjos, paddle faster”")),
          Oe.forEach(i),
          (xe = w(
            q,
            " out of my minds eye. After slapping on the tire, kicking tight the lug nuts with my tire iron, tossing my destroyed tire where the spare used to live, we made for the nearest gas station where things started to calm down."
          )),
          q.forEach(i),
          (se = _(t)),
          (H = k(t, "P", {}));
        var Re = c(H);
        (Ee = w(
          Re,
          "We decided to post up for the night once we made it to Lexington, Kentucky. The rest of the trip was honestly, thankfully, uneventful. During this trip, I was awe-struck at the incomprehensible vastness of the United States. I often rescinded into my mind as I imagined the fears and troubles colonial settlers must have experienced going Westward, and how wantonly they discarded and destroyed Native American wisdom and culture along the way in their self-appointed “manifest destiny”. But eventually, we made it— we were finally in Denver."
        )),
          Re.forEach(i),
          (le = _(t)),
          (W = k(t, "H2", { id: !0 }));
        var Se = c(W);
        j = k(Se, "A", { class: !0, title: !0, "aria-hidden": !0, href: !0 });
        var Ue = c(j);
        Q = k(Ue, "SPAN", {});
        var Fe = c(Q);
        (Ae = w(Fe, "#")),
          Fe.forEach(i),
          Ue.forEach(i),
          (De = w(Se, "Denver and Co")),
          Se.forEach(i),
          (he = _(t)),
          (T = k(t, "P", {}));
        var ge = c(T);
        (je = w(
          ge,
          "I am still awe-struck at the natural beauty of Colorado and its incredible biodiversity. Denver itself is surrounded on all sides by gorgeous mountains, which of course prompts many outdoors-loving individuals to experience nature through “14ers”. A “14er” is a mountain that ascenders to 14,000’ or higher in elevation and let me tell ya— there is no shortage of mountains to choose from in Colorado. One of the most memorable experiences during our time in Denver was conquering my first 14er, "
        )),
          (P = k(ge, "A", { href: !0, rel: !0 }));
        var Ze = c(P);
        (Me = w(Ze, "Mount Sherman")),
          Ze.forEach(i),
          (Ve = w(
            ge,
            ". The bottom of the trail is home to several abandoned mining stations, all rusted and decayed from decades of disuse, until the easily walkable dirt trails give way to a sea of stacked flat stones that make each step a calculated effort. I had forgotten what it was like to truly push myself to my physical limit since I stopped playing Rugby, but near the summit, my body began to give up as fatigue and colorie deficiency set in. A peanut-butter bar and a gatorade later, I joined Kristen for the last leg of the trek until we had finally made it to the top. As I stood at the summit, gasping for air in the thin mountain atmosphere, I felt a sense of accomplishment unlike anything I had ever experienced before. The view from the top was breathtaking, with endless peaks stretching out before me in every direction. I could feel the sun on my face and the wind in my hair, and for the half-hour we spent resting at the summit, all the challenges of the move seemed a distant afterthought."
          )),
          ge.forEach(i),
          (de = _(t)),
          F(K.$$.fragment, t),
          (ue = _(t)),
          (O = k(t, "P", {}));
        var Je = c(O);
        (We = w(
          Je,
          "Despite this, and despite our accomplishments, we found ourselves missing Virginia and the DMV area. We especially felt bad for our cats, who went from living as indoor/outdoor cats in a part of VA where they had no natural predators and could roam safely outside, with my trust that they’d come home when called, something that I trained them to do over the first 6 months of their lives with us. They were now confined to an apartment, less than 1000 square feet in size, with just me to keep them company as a remote worker. I also began to quickly miss the cultural diversity afforded to us by being so close to Washington D.C. and having immediate access to the truly world-class Metro in the DMV. But it was done and we had sunk too much money to have regrets, so we settled in, and spent 2 wonderful years in Colorado before it was off to the next adventure as Kristen transitioned into being a travelling Nurse. Now we’re in Arizona and soon after, we’ll be in Oregon. Who knows what’s next, but one thing is certain: I’m so glad we moved. If we hadn’t, we’d still likely be stuck in that black hole of a state, and I wouldn’t have conquered my first 14er, or learned how to fall on my ass 23 different ways while learning to ski."
        )),
          Je.forEach(i),
          (ce = _(t)),
          F(z.$$.fragment, t),
          (fe = _(t)),
          (R = k(t, "P", {}));
        var Ye = c(R);
        (Te = w(
          Ye,
          "Looking back on our decision to move to Denver, I realize now that it was about more than just escaping the b”lack hole of the East Coast”. It was about pushing ourselves out of our comfort zones, embracing new experiences, and discovering what we were truly capable of. The move tested our relationship, our resilience, and our sense of adventure, but in the end, it brought us closer together and taught us valuable lessons about ourselves and each other. To anyone considering a similar life change, I would say this: Don’t let fear hold you back. Embrace the unknown, take risks, and trust in your ability to adapt and grow. You never know what incredible experiences and opportunities await you on the other side."
        )),
          Ye.forEach(i),
          this.h();
      },
      h() {
        n(
          o,
          "href",
          "https://abcnews.go.com/Business/home-prices-soaring-bubble/story?id=110101842"
        ),
          n(o, "rel", "nofollow"),
          n(D, "class", "heading-link"),
          n(D, "title", "Permalink"),
          n(D, "aria-hidden", "true"),
          n(D, "href", "#the-move"),
          n(V, "id", "the-move"),
          n(
            S,
            "href",
            "https://en.wikipedia.org/wiki/Mothman#:~:text=Mothman%2C%20in%20West%20Virginian%20folklore,Creature%20..."
          ),
          n(S, "rel", "nofollow"),
          n(B, "href", "https://www.youtube.com/watch?v=pDlZLsJJkVA"),
          n(B, "rel", "nofollow"),
          n(j, "class", "heading-link"),
          n(j, "title", "Permalink"),
          n(j, "aria-hidden", "true"),
          n(j, "href", "#denver-and-co"),
          n(W, "id", "denver-and-co"),
          n(
            P,
            "href",
            "https://www.outdoortrailmaps.com/mount-sherman-14036-ft/"
          ),
          n(P, "rel", "nofollow");
      },
      m(t, u) {
        f(t, e, u),
          l(e, a),
          l(e, o),
          l(o, r),
          l(e, s),
          f(t, d, u),
          f(t, y, u),
          l(y, h),
          f(t, g, u),
          Z(I, t, u),
          f(t, ae, u),
          f(t, V, u),
          l(V, D),
          l(D, Y),
          l(Y, pe),
          l(V, we),
          f(t, ne, u),
          f(t, L, u),
          l(L, ye),
          f(t, ie, u),
          f(t, N, u),
          l(N, ve),
          f(t, re, u),
          f(t, $, u),
          l($, ke),
          l($, S),
          l(S, be),
          l($, _e),
          l($, B),
          l(B, Ie),
          l($, $e),
          l($, G),
          l(G, Ce),
          l($, xe),
          f(t, se, u),
          f(t, H, u),
          l(H, Ee),
          f(t, le, u),
          f(t, W, u),
          l(W, j),
          l(j, Q),
          l(Q, Ae),
          l(W, De),
          f(t, he, u),
          f(t, T, u),
          l(T, je),
          l(T, P),
          l(P, Me),
          l(T, Ve),
          f(t, de, u),
          Z(K, t, u),
          f(t, ue, u),
          f(t, O, u),
          l(O, We),
          f(t, ce, u),
          Z(z, t, u),
          f(t, fe, u),
          f(t, R, u),
          l(R, Te),
          (me = !0);
      },
      p(t, [u]) {
        const X = {};
        u & 1 && (X.$$scope = { dirty: u, ctx: t }), I.$set(X);
      },
      i(t) {
        me ||
          (C(I.$$.fragment, t),
          C(K.$$.fragment, t),
          C(z.$$.fragment, t),
          (me = !0));
      },
      o(t) {
        A(I.$$.fragment, t),
          A(K.$$.fragment, t),
          A(z.$$.fragment, t),
          (me = !1);
      },
      d(t) {
        t && i(e),
          t && i(d),
          t && i(y),
          t && i(g),
          J(I, t),
          t && i(ae),
          t && i(V),
          t && i(ne),
          t && i(L),
          t && i(ie),
          t && i(N),
          t && i(re),
          t && i($),
          t && i(se),
          t && i(H),
          t && i(le),
          t && i(W),
          t && i(he),
          t && i(T),
          t && i(de),
          J(K, t),
          t && i(ue),
          t && i(O),
          t && i(ce),
          J(z, t),
          t && i(fe),
          t && i(R);
      },
    }
  );
}
class bt extends ee {
  constructor(e) {
    super(), te(this, e, null, yt, oe, {});
  }
}
export { bt as component };
